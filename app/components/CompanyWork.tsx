"use client";

import Link from "next/link";
import Image from "next/image";
import { motion } from "framer-motion";
import { FaLocationArrow } from "react-icons/fa6";
import { PinContainer } from "./ui/3d-pin";
import { IconRow } from "./ui/BrandIcon";
import { urlForOptimized } from "@/sanity/lib/image";
import type { CompanyWorkQueryResult } from "@/sanity/lib/types";

interface CompanyWorkProps {
  data: CompanyWorkQueryResult;
}

const EMPLOYER_ORDER = ["MageMontreal", "Arctic Leaf"];

const CompanyWork = ({ data }: CompanyWorkProps) => {
  if (!data || data.length === 0) {
    return (
      <div className="py-20" id="agency-work">
        <h2 className="heading pb-7">
          Agency Work:{" "}
          <span className="dark:text-purple text-blue-500">
            Projects Built on the Job
          </span>
        </h2>
        <p className="text-center text-gray-600 dark:text-gray-400">No agency work found.</p>
      </div>
    );
  }

  const grouped = EMPLOYER_ORDER.reduce<Record<string, CompanyWorkQueryResult[number][]>>(
    (acc, employer) => {
      acc[employer] = data.filter((item) => item.employer === employer);
      return acc;
    },
    {}
  );

  return (
    <div className="py-20" id="agency-work">
      <motion.h2
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
        className="heading pb-7"
      >
        Agency Work:{" "}
        <span className="dark:text-purple text-blue-500">
          Projects Built on the Job
        </span>
      </motion.h2>

      {EMPLOYER_ORDER.map((employer) => {
        const projects = grouped[employer];
        if (!projects || projects.length === 0) return null;

        return (
          <div key={employer} className="mb-16">
            <h3 className="text-2xl font-semibold mb-8 text-center text-gray-700 dark:text-gray-300">
              {employer}
            </h3>
            <div className="flex flex-wrap items-center justify-center p-4 gap-20">
              {projects.map((item, index) => (
                <motion.div
                  key={item._id}
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: index * 0.1 }}
                  className="lg:min-h-[40rem] h-[25rem] flex items-center justify-center sm:w-[28rem] w-[80vw]"
                >
                  <PinContainer title={item.pinTitle} href={item.link}>
                    <Link href={item.link} target="_blank">
                      <div className="relative flex items-center justify-center sm:w-[28rem] w-[80vw] h-[20vh] lg:h-[35vh] mb-10 rounded-md overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700 animate-pulse" />
                        <Image
                          src={urlForOptimized(item.img, { width: 400, height: 300, quality: 85 }).url()}
                          alt="cover"
                          className="rounded-md object-cover transition-opacity duration-500"
                          style={{ opacity: 0 }}
                          onLoad={(e) => { e.currentTarget.style.opacity = "1"; }}
                          fill
                          priority={index < 3}
                        />
                      </div>

                      <h1 className="font-bold lg:text-2xl md:text-xl text-base line-clamp-1">
                        {item.title}
                      </h1>

                      <p
                        className="lg:text-xl lg:font-normal font-light text-sm line-clamp-2 dark:text-gray-400 text-black"
                        style={{ margin: "1vh 0" }}
                      >
                        {item.desc}
                      </p>

                      <div className="flex items-center justify-between mt-7 mb-3">
                        <div className="flex items-center">
                          <IconRow
                            slugs={item.iconSlugs.map((tech) => tech.iconSlug)}
                            size={32}
                            className="flex gap-2"
                          />
                        </div>
                        <div className="flex justify-center items-center">
                          <p className="flex lg:text-xl md:text-xs text-sm dark:text-purple text-blue-500">
                            Check Live Site
                          </p>
                          <FaLocationArrow className="ms-3 text-blue-500 dark:text-purple" />
                        </div>
                      </div>
                    </Link>
                  </PinContainer>
                </motion.div>
              ))}
            </div>
          </div>
        );
      })}
    </div>
  );
};

export default CompanyWork;
